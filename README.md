...
'리뷰메이트(ReviewMate)'는 사용자의 고유한 문체를 학습하여 '나다운' 리뷰 초안을 생성하고, AI가 사용자의 선호도를 예측하여 최적의 결과물을 추천하는 초개인화 AI 텍스트 생성 보조 서비스입니다. 온라인 리뷰 작성의 피로감, 대가성 리뷰로 인한 신뢰도 하락, 기존 AI 생성 텍스트의 어색함이라는 문제를 해결하여, 사용자가 **시간은 절약**하면서도 **진정성 있는('나다운')** 리뷰를 손쉽게 작성하도록 돕는 것을 목표로 합니다.

본 프로젝트는 한양대학교 창업지원단 지원 대상으로 선정되었으며, 초기 **`SwiftUI`** 기반 iOS 앱 기획에서 더 빠른 시장 검증과 플랫폼 확장성을 위해 **`Next.js`** 웹 애플리케이션으로 전략적으로 피벗하여 개발을 진행 중입니다.

---

## 🏗️ 시스템 아키텍처 (System Architecture)

'리뷰메이트'는 다음과 같은 주요 구성 요소로 설계되었습니다:

1.  **Frontend (Next.js Web App):**
    * 사용자가 키워드 입력, 문체 샘플 제공, 생성된 리뷰 확인 및 피드백을 제공하는 메인 인터페이스입니다.
    * `TypeScript`, `React`, `Tailwind CSS`를 사용하여 개발됩니다.
2.  **Backend (Next.js API Routes / Server Actions):**
    * 프론트엔드로부터 요청을 받아 AI 모델 API를 호출하고, 결과를 처리하여 반환합니다.
    * 사용자 데이터, 문체 벡터, 피드백 등을 데이터베이스에 저장/관리합니다.
3.  **AI Models (External APIs & Custom Logic):**
    * **Text Generation:** `Clova LLM` (초기) 또는 `OpenAI API` (Fine-tuning 고려)를 활용하여 기본 리뷰 초안을 생성합니다.
...
4.  **Database (Vercel Postgres + Prisma):**
    * 사용자 정보, 학습된 문체 데이터, 생성된 리뷰, 피드백 데이터 등을 저장합니다. `Prisma ORM`을 통해 관리됩니다.
5.  **Hosting (Vercel):**
    * `Next.js` 애플리케이션 배포 및 관리를 위해 Vercel 플랫폼을 사용합니다.
...
문체 학습 및 최적 추천 기능을 구현하고 서비스를 안정화합니다.
...
마켓플레이스, 구독 모델 등 부가 기능을 구현하고 수익 모델을 도입합니다.
...
*(역할은 프로젝트 진행에 따라 유연하게 조정될 수 있습니다.)*